// Vercel serverless function - exports Express app with all routes
// This is the complete backend API for KPI Vault
...
const ensureDataFile = () => {
  if (!existsSync(DATA_FILE)) {
    ...
    if (!existsSync(dir)) {
      // Create directory if it doesn't exist (for /tmp)
      try {
...
// CORS - allow KPI Vault frontend and local development
const corsOptions = {
  origin: [
    'https://kpi-vault.zamataskhub.com', // KPI Vault Frontend (Cloudflare Tunnel)
    /\.vercel\.app$/, // Allow Vercel deployments (for testing)
    /\.netlify\.app$/, // Allow Netlify deployments (for testing)
...
app.patch('/feedback/:feedbackId', (req, res) => {
  ...
  if (commitment) entry.commitment = commitment;
  if (noteMetadata) entry.noteMetadata = noteMetadata;
...
app.post('/alerts/:alertId/trigger', (req, res) => {
  ...
  const workerAuthorized = process.env.ALERT_WORKER_KEY
    ? workerKeyHeader === process.env.ALERT_WORKER_KEY
    : !!workerKeyHeader;
